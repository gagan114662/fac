#!/usr/bin/env python3
"""Emit a structured fac telemetry event when the local daemon is available."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Emit a structured fac event")
...
def daemon_base_url() -> str:
    explicit = os.environ.get("FAC_API_BASE", "").strip()
    if explicit:
        return explicit.rstrip("/")

    daemon_info = Path.home() / ".fac" / "daemon.json"
